...
def rapport_node(state: SessionState) -> dict:
    ...
    if step == "who":
        ...
        try:
            # Scripted rapport uses the same fast empathy profile as the loop.
...
def values_node(state: SessionState) -> dict:
    ...
    # This invocation ends after the values node.  Emit the first rapport
    # message here so callers do not accidentally redisplay the previous
    # value-selection prompt while the internal stage has already advanced.
    message = _rapport_question("greeting", state["name"])
    return {
