"""Gemini model calls shared by the baseline/optimized pipeline experiment."""
...
_TASK_JSON_SCHEMAS: dict[str, dict] = {
    ...
    "extract_slots": {
        "type": "object",
        "properties": {key: _NULLABLE_STRING for key in _SLOT_KEYS},
        "required": _SLOT_KEYS,
        "additionalProperties": False,
    },
    "sufficiency_check": {
        ...
        "properties": {
            ...
            "missing_aspect": _NULLABLE_STRING,
        },
        "required": ["sufficient", "missing_aspect"],
        "additionalProperties": False,
    },
        ...
        "properties": {
            ...
            "incidental_updates": {
                ...
                "items": {
                    ...
                    "required": ["slot", "value", "sufficient", "confidence"],
                    "additionalProperties": False,
                },
            },
        },
    ...
    "response_candidates": {
        ...
        "properties": {
            ...
            "if_sufficient": {"type": "string"},
            "if_insufficient": {"type": "string"},
        },
        "required": ["reflection", "if_sufficient", "if_insufficient"],
        "additionalProperties": False,
    },
    "consolidate_slots": {
        "type": "object",
...
def _mock_json(
    ...
    if task in {"next_question", "detail_question"}:
        goal_match = re.search(
            r"(?:다음으로 확인할 내용|슬롯 의도):\s*([^\n]+)", user
        )
        goal = goal_match.group(1).strip() if goal_match else "조금 더 들려주실 수 있을까요?"
        return {"question": f"{goal}에 대해 편하게 이야기해 주실 수 있을까요?"}
    if task == "extract_slots":
        match = re.search(r"사용자 마지막 발화:\s*(.*?)(?:\n\n|$)", user, re.DOTALL)
        utterance = (match.group(1).strip() if match else user.strip())[:200]
        target_match = re.search(r"이번 턴에 답변 대상이 된 슬롯:\s*([a-z_]+)", user)
        target = target_match.group(1) if target_match and target_match.group(1) in _SLOT_KEYS else "situation"
        result = {key: None for key in _SLOT_KEYS}
        result[target] = utterance or None
        return result
    if task == "sufficiency_check":
        return {"sufficient": True, "missing_aspect": None}
    if task in {"target_slot_analysis", "baseline_turn_analysis"}:
        ...
        result = {
            ...
            "missing_aspect": None if value else "답변 없음",
            "confidence": 0.95,
            "incidental_updates": [],
        }
        if task == "baseline_turn_analysis":
            result.pop("missing_aspect", None)
            result.pop("confidence", None)
        return result
    if task == "response_candidates":
        return {
            "reflection": "말씀해 주신 마음이 느껴져요.",
            "if_sufficient": "다음 이야기도 편하게 들려주실 수 있을까요?",
            "if_insufficient": "그 부분을 조금만 더 구체적으로 들려주실 수 있을까요?",
        }
...
def call_model_json(
    ...
    *,
    ...
    for invalid JSON or missing required fields, and each attempt is included in the
    A/B metrics.
    """
    ...
    mock_mode = _env_bool("AB_MOCK_MODE", False)
    ...
    _append_trace(
        {
            ...
            "timed_out": timed_out,
            # Language leakage is a quality signal, not an arm-specific retry
            # condition. Applying the same observation to both arms keeps latency
            # and success-rate comparisons fair.
...
def provider_status(probe: bool = False) -> dict[str, dict]:
    ...
    analyzer_config = _config("gemini", "analyzer")
    ...
    entry = {
        ...
        "profiles": {
            ...
            "optimized": {
                ...
                "analyzer_api_route": "analyzer" if analyzer_separate else "primary",
                "analyzer_thinking_level": os.getenv(
                    "OPTIMIZED_ANALYZER_REASONING_EFFORT"
                )
                or os.getenv("OPTIMIZED_ANALYZER_THINKING", "low").strip(),
                "analyzer_api_separate": analyzer_separate,
                "analyzer_configured": _configured(analyzer_config) or mock_mode,
            },
        },
    }
    if config.auth_mode == "vertex_adc":
