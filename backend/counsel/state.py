...
PipelineArm = Literal["baseline", "optimized"]
ModelProvider = Literal["gemini"]
SlotSwitch = Literal["off", "asking", "on", "unknown"]

# Only the six slots the mind map shows are collected. The other five
# (cause, duration, impact, relationship, self_message) are commented out
# throughout this module, counsel/llm.py and counsel/prompts.py rather than
# deleted, so the longer flow can be restored by uncommenting.
# compute_gate() already gated on exactly these six, so the loop now ends when
# the last one it asks for is filled.
SLOT_ORDER = [
    "situation",
    "emotion",
    "thought",
...
# Deterministic user-facing fallbacks for the optimized arm.  The optimized
# response writer speculates about the next slot before analysis completes; an
# incidental slot update can change that target, so the pre-generated question
# is not always safe to reuse.
